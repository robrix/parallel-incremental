...
instance Corecursive1 r => Functor (r (Grammar t)) where
  fmap = liftA

instance Corecursive1 r => Applicative (r (Grammar t)) where
  pure = embed1 . Nul
  liftA2 f a b = embed1 (Seq f a b)

instance (Corecursive1 r, Recursive (r (Grammar t))) => Alternative (r (Grammar t)) where
  empty = embed1 (Err [])
  a <|> b = embed1 (Alt a b)
  many a = mu (\ more -> (:) <$> a <*> more <|> pure [])
  some a = (:) <$> a <*> many a

instance (Corecursive1 r, Recursive (r (Grammar t))) => Parsing (r (Grammar t)) where
  try = id
  a <?> s = embed1 (Lab a s)
  eof = embed1 (End ())
...
instance (Corecursive1 r, Recursive (r (Grammar Char))) => CharParsing (r (Grammar Char)) where
  satisfy p = embed1 (Sat (\ c -> guard (p c) *> Just c))

instance (Corecursive1 r, Recursive (r (Grammar Char))) => TokenParsing (r (Grammar Char))
